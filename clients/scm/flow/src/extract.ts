...
function buildServicePrompt(rawMessage: string, history: HistoryMessage[]): string {
  return [
    ...
    "",
    "Valid service keys (return EXACTLY one of these):",
    '- "free_eligibility" — Free 15-min Eligibility Consultation ($0)',
    '- "trt_initial" — TRT Initial Consultation ($179, 30 min)',
    '- "trt_followup" — TRT Follow-up ($159, 20 min)',
    '- "ed_initial" — ED Express Appointment ($179, 30 min)',
    '- "glp1_initial" — GLP-1 Initial Consultation ($119, 20 min)',
    '- "weightmgmt_initial" — Weight Management Initial ($159, 30 min)',
    '- "roidcare_initial" — RoidCare+ Initial ($179, 30 min)',
    '- "nutrition_initial" — Nutrition Initial ($159, 30 min)',
    '- "hairloss_initial" — Hairloss Initial ($159, 30 min)',
    "",
    "Routing rules:",
    '- If the patient asks for "free consult", "free eligibility", "free check", "eligibility check", or anything free → return "free_eligibility"',
    '- If unsure which specific paid service → return "free_eligibility" (safer default)',
    "",
    "Conversation history:",
    ...history.map((h) => `${h.role}: ${h.content}`),
    "",
