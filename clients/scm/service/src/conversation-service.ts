...
import type { createGhlClient, GhlMessagePayload } from "@romea/ghl-client";
...
interface StoredSendPayload extends GhlMessagePayload {
  locationId: string;
}
...
async function incrementSendAttempts(
  db: Pool,
  messageId: string,
): Promise<void> {
  await db.query(
...
/* ------------------------------------------------------------------ */
/*  Recovery — at-least-once reply delivery                            */
/* ------------------------------------------------------------------ */

interface UnsentReplyRow {
  message_id: string;
  contact_id: string;
  send_payload: StoredSendPayload;
...
export async function recoverUnsentReplies(
  ...
  for (const row of result.rows) {
    try {
      ...
      await ghl.sendMessage(locationId, row.contact_id, payload);
